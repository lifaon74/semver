...
import { ISemVer } from '../../semver/types/semver.type.js';
...
export function testSemVerRange(input: ISemVerRange, semver: ISemVer): boolean {
  switch (input.type) {
    case 'union':
...
function testSemVerUnionRange(input: ISemVerUnionRange, semver: ISemVer): boolean {
  return input.ranges.some((input: ISemVerRange): boolean => {
    return testSemVerRange(input, semver);
  });
}

function testSemVerIntersectionRange(input: ISemVerIntersectionRange, semver: ISemVer): boolean {
  return input.ranges.every((input: ISemVerRange): boolean => {
    return testSemVerRange(input, semver);
  });
}

function testSemVerHyphenatedRange(input: ISemVerHyphenatedRange, semver: ISemVer): boolean {
  return (
    testPartialSemVer(input.left, semver, '>=') && testPartialSemVer(input.right, semver, '<=')
  );
}

function testSemVerSimpleRange(input: ISemVerSimpleRange, semver: ISemVer): boolean {
  return testPartialSemVer(input.semver, semver, input.operator === '' ? '=' : input.operator);
}

function testSemVerShortcutRange(input: ISemVerShortcutRange, semver: ISemVer): boolean {
  switch (input.operator) {
    case '~': {
      if (isPartialSemVerWildcard(input.semver.major)) {
        ...
        return (
          compareSemVer(semver, input.semver as ISemVer) >= 0 &&
          compareSemVer(semver, {
            major: input.semver.major,
            minor: input.semver.minor + 1,
            patch: 0,
...
function testPartialSemVer(
  ...
  if (isPartialSemVerWildcard(partialSemVer.major)) {
    ...
    const r: number = compareSemVer(semver, partialSemVer as ISemVer);
    switch (operator) {
      case '=':
        return r === 0;
      case '>':
